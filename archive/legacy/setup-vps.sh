...
# Use the committed nginx.conf.template from git (infrastructure/nginx/).
# DO NOT regenerate inline — the repo template has rate limiting, security
# headers, upstream keepalive, /send-message, and ACME challenge support.
if [ ! -f "$NGINX_TEMPLATE" ]; then
    err "nginx.conf.template ausente em $NGINX_TEMPLATE"
    err "Verifique se o git clone está completo."
    exit 1
fi
