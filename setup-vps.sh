...
# ZAPFLOW AI — Production Nginx Config (generated by setup-vps.sh)
# Proxies ALL backend routes through Docker network DNS.
...
server {
    ...
    # MUST come AFTER all proxy locations so API calls are proxied first.
    location / {
        try_files $uri $uri/ /index.html;
        # Prevent browser caching SPA HTML (it contains JS chunk references)
    ...
    # ── Public health/status endpoints (no auth) ─────────────
    location = /health {
        proxy_pass http://${BACKEND_SERVICE}:4025;
        proxy_http_version 1.1;
        proxy_set_header Host $host;
